...
function searchGithub(term) {
    console.log('searchGithub', term);

    var promise = $.getJSON('https://api.github.com/search/users?q=' + term).promise();

    return Rx.Observable.fromPromise(promise)
        .map(function(request) {
            return request.items
                .map(function(item) { return item.login; });
        });
}

$(function() {
  Rx.Observable
    .fromEvent($("#autocomplete"), "keyup")
    .throttle(400)
